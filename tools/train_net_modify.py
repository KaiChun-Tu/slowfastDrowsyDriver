...
def get_F1Score(preds, labels):
    ...
    logger.info("Getting F1 score for {} examples".format(preds.shape[0]))
    #preds = np.argmax(preds,1)
    #labels = labels[:, ~(np.all(labels == 0, axis=0))]
    precision = (precision_score(labels, preds, average='macro'))
    recall = (recall_score(labels, preds, average='macro'))
    f1_score = (average_precision_score(labels, preds))
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        train_meter.iter_tic()
    # Log epoch stats.
    # result_write = open('train_result.txt','a')
    # result_write.write('epoch = '+str(cur_epoch))
    # result_write.write(' loss = '+str(loss))
    # result_write.write('\n')
    # result_write.close()
...
@torch.no_grad()
def eval_epoch(val_loader, model, val_meter, cur_epoch, cfg,optimizer, writer=None):
    """
    ...
    if f1_score >best_f1 :
        best_f1 =f1_score
        best_epoch = int(cur_epoch)
        cu.save_bestF1_checkpoint(
            cfg.OUTPUT_DIR,
            model,
            optimizer,
            cur_epoch,
...
def train(cfg):
    ...
    Args:
        cfg (CfgNode): configs. Details can be found in
            slowfast/config/defaults.py
    """
    result_write = open('train_result.txt','w')
    result_write.close()
    ...
    for cur_epoch in range(start_epoch, cfg.SOLVER.MAX_EPOCH):
        ...
        if is_eval_epoch:
            eval_epoch(val_loader, model, val_meter, cur_epoch, cfg,optimizer, writer)
