...
class ResStage(nn.Module):
    ...
    def __init__(
        ...
        # self.att_conv33_13 = AugmentedConv_time(in_channels=dim_out[0], out_channels=dim_out[0], kernel_size=3, dk=40, dv=4, Nh=4,
        #                                    relative=True,
        #                                    stride=1, shape=image_size)
    ...
    def forward(self, inputs):
        ...
        for pathway in range(self.num_pathways):
            ...
            #         temp = x[:,:,t,:,:]
            #         temp = self.att_conv33_13(temp)
            #         tmp += [temp]
            #     x = torch.stack(tmp,axis = 2)
            #---------------------------
            #self Attention time
            # if pathway == 0:
...
class AugmentedConv_time(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, dk, dv, Nh, shape=0, relative=False, stride=1):
        super(AugmentedConv_time, self).__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.kernel_size = kernel_size
        self.dk = dk
    ...
    def forward(self, x):
        ...
        for t in range(tNum):
            xt = x[:,:,t,:,:]
            conv_out = self.conv_out(xt)
            conv_outTList = conv_outTList+[conv_out]
            batch, _, height, width = conv_out.size()
            ...
            # q, k, v
            # (batch_size, Nh, height, width, dv or dk)
            flat_q, flat_k, flat_v, q, k, v = self.compute_flat_qkv(xt, self.dk, self.dv, self.Nh)
            flat_qTList = flat_qTList+[flat_q]
            flat_kTList = flat_kTList+[flat_k]
            flat_vTList = flat_vTList+[flat_v]
        flat_q = self.calculateAverage(flat_qTList)  
        flat_k = self.calculateAverage(flat_kTList)
        flat_v = self.calculateAverage(flat_vTList) 
        logits = torch.matmul(flat_q.transpose(2, 3), flat_k)
        if self.relative:
            h_rel_logits, w_rel_logits = self.relative_logits(q)
            logits += h_rel_logits
        ...
        attn_out = self.attn_out(attn_out)
        for t in range(tNum):
            xt = torch.cat((conv_outTList[t], attn_out), dim=1)
            xtList = xtList+[xt.unsqueeze(2)]
        return torch.cat(xtList, dim=2)

    def calculateAverage(self,tensorList):
        average = torch.stack(tensorList)
        average = torch.mean(average, 0) 
        return average
