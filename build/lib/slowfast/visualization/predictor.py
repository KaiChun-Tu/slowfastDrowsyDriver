...
class Predictor:
    ...
    def __init__(self, cfg, gpu_id=None):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            self.object_detector = PFLDPredictor(cfg, gpu_id=self.gpu_id)
    ...
    def __call__(self, task):
        ...
        # cv2.imshow('inputs',torch.from_numpy(np.array(frames[0])).float() / 255)
        # cv2.waitKey(0)
        if bboxes is not None:
...
class PFLDPredictor:
    """
    Wrapper around PFlD to return the required predicted bounding boxes
    as a ndarray.
    """
    ...
    def __init__(self, cfg, gpu_id=None):
        ...
        #self.face_model = AntiSpoofPredict(0)
        self.cfg = get_cfg()
        self.cfg.merge_from_file(
            model_zoo.get_config_file(cfg.DEMO.DETECTRON2_CFG)
        )
    ...
    def __call__(self, task):
        ...
        """
        middle_frame = task.frames[len(task.frames) // 2]
        #image_bbox = self.face_model.get_bbox(middle_frame)
        #x1 = float(image_bbox[0])
        #y1 = float(image_bbox[1])
        #x2 = float(image_bbox[0] + image_bbox[2])
        #y2 = float(image_bbox[1] + image_bbox[3])
        #image_bbox = torch.tensor([[x1,y1,x2,y2]]).cuda()
        #outputs = self.predictor(middle_frame)
        # Get only human instances
        #mask = outputs["instances"].pred_classes == 0
        #pred_boxes = outputs["instances"].pred_boxes.tensor[mask]
        task.add_bboxes(image_bbox)
